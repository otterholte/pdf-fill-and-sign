...
- Distinguish independent square controls, circular choices, and explicit “check only one” groups. Validate fallback boxes against their outlines.
...
Run `npm test`, `npm run test:browser`, and `npm run test:dense`. Windows can select installed Edge with `BROWSER_CHANNEL=msedge`. CI runs Chromium on Linux. Set `TEST_BASE_URL` to the deployed site to test production; both browser suites first verify that the deployed source matches the checkout exactly.
